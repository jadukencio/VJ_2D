...
bool TileMap::loadLevel(const string &levelFile)
{
	...
	for(int j=0; j<mapSize.y; j++)
	{
		...
		string word;
		int i = 0;
		while (!ss.eof() && i<mapSize.x) {
			getline(ss, word, del);
			cout << "TILE " + i + ',' + j + ':' << stoi(word) << endl;
			map[j * mapSize.x + i] = stoi(word) + 1;
			i++;
		}
		/*
	...
	for(int j=0; j<mapSize.y; j++)
	{
		for(int i=0; i<mapSize.x; i++)
		{
			...
			if(tile != 0)
			{
				// Non-empty tile
				nTiles++;
				posTile = glm::vec2(minCoords.x + offset + i * tileSize, minCoords.y + j * tileSize);
...
bool TileMap::collisionMoveDown(const glm::ivec2& pos, const glm::ivec2& size) const
{
	...
	x0 = pos.x / tileSize;
	x1 = (pos.x + size.x - 1) / tileSize;
	y = (pos.y + size.y + 8 - 1) / tileSize;
	for (int x = x0; x <= x1; x++)
	{
		if (map[y * mapSize.x + x] > 100)
			return true;
	}
